    ...
    return (
        <article className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-4 md:flex-row">
                <div className="h-28 w-28 overflow-hidden rounded-xl bg-slate-100 md:h-32 md:w-32">
                    {profile.image_url ? (
                            ...
                            <div>
                                ...
                                <div className="mt-1 flex flex-wrap gap-2">
                                    {slots.morning.map((slot) => (
                                        <span key={slot} className="rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs text-slate-700">
                                            {slot}
                                        </span>
                                    ))}
                                </div>
                    ...
                    <div className="mt-4 space-y-2">
                        {previewServices.length > 0 ? (
                            previewServices.map((service) => (
                                <div
                                    key={service.id}
                                    className="flex flex-col justify-between gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 sm:flex-row sm:items-center"
                                >
                                    <div>

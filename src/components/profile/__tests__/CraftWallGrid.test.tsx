...
describe('CraftWallGrid', () => {
    ...
    it('returns null when photos array is empty', () => {
        const { container } = renderWithIntl(<CraftWallGrid photos={[]} />);
        expect(container.innerHTML).toBe('');
    });

    it('renders all photos in a grid', () => {
        const { container } = renderWithIntl(<CraftWallGrid photos={makePhotos(6)} />);
        const imgs = container.querySelectorAll('img');
        expect(imgs).toHaveLength(6);
    });

    it('applies correct responsive grid classes by default (4 cols)', () => {
        const { container } = renderWithIntl(<CraftWallGrid photos={makePhotos(4)} />);
    ...
    it('applies 2-column grid when columns=2', () => {
        const { container } = renderWithIntl(<CraftWallGrid photos={makePhotos(4)} columns={2} />);
        const grid = container.querySelector('.grid');
        expect(grid?.className).toContain('grid-cols-2');
    ...
    it('uses lazy loading for photos beyond index 8', () => {
        const { container } = renderWithIntl(<CraftWallGrid photos={makePhotos(12)} />);
        const imgs = container.querySelectorAll('img');
    ...
    it('fires onPhotoTap with the correct photo and index', () => {
        const onTap = vi.fn();
        renderWithIntl(<CraftWallGrid photos={makePhotos(4)} onPhotoTap={onTap} />);
    ...
    it('shows "Показать ещё" when maxInitialRows limits visible photos', () => {
        const { container } = renderWithIntl(<CraftWallGrid photos={makePhotos(20)} maxInitialRows={2} />);
        const imgs = container.querySelectorAll('img');
    ...
    it('hides "Показать ещё" when all photos are visible', () => {
        renderWithIntl(<CraftWallGrid photos={makePhotos(4)} maxInitialRows={2} />);
    ...
    it('shuffles photos deterministically with shuffleSeed', () => {
        ...
        const { container: c1 } = renderWithIntl(<CraftWallGrid photos={photos} shuffleSeed={42} />);
        const order1 = Array.from(c1.querySelectorAll('img')).map((img) =>
            img.getAttribute('src')
        );

        const { container: c2 } = renderWithIntl(<CraftWallGrid photos={photos} shuffleSeed={42} />);

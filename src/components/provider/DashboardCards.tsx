...
// Card import removed as it was incorrect and unused
...
export function ServicePublicationCard() {
    return (
        <div className="bg-[#0044BB] text-white p-6 rounded-2xl shadow-lg mb-6 overflow-hidden relative">
            {/* Abstract background effect could go here */}
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                {service.status === 'REJECTED' && (
                                    <span className="bg-red-100 text-red-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Отклонена</span>
                                )}
                            </div>
                            <Link href={`/services/${service.id}`} className="block after:absolute after:inset-0">
                                <h4 className="font-bold text-lg text-gray-900 group-hover:text-blue-600 transition-colors">
                                    {(() => {
                                        // Attempt to clean title if it contains JSON-like array
                                        if (service.title.includes('[{"name":')) {
                                            try {
                                                // Extract the JSON part. Assuming format "Title - JSON"
                                                const parts = service.title.split(' - ');
                                                if (parts.length > 1) {
                                                    const potentialJson = parts[1]; // Or join rest?
                                                    // Try to find the start of JSON array
                                                    const jsonStartIndex = service.title.indexOf('[');
                                                    if (jsonStartIndex > -1) {
                                                        const prefix = service.title.substring(0, jsonStartIndex).replace(/ - $/, '');
                                                        const jsonStr = service.title.substring(jsonStartIndex);
                                                        const parsed = JSON.parse(jsonStr);
                                                        if (Array.isArray(parsed) && parsed.length > 0 && parsed[0].name) {
                                                            return `${prefix} - ${parsed[0].name}`;
                                                        }
                                                    }
                                                }
                                            } catch (e) {

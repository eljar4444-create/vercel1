...
export async function generateMetadata({
    ...
    if (query && city) {
        title = t('metaTitleQueryCity', { query, city });
        description = t('metaDescQueryCity', { query, city });
    } else if (query) {
        title = t('metaTitleQuery', { query });
        description = t('metaDescQuery', { query });
    } else if (city) {
        title = t('metaTitleCity', { city });
        description = t('metaDescCity', { city });
    } else {
        title = t('metaTitle');
...
export default async function SearchPage({
    ...
    return (
        <main className="h-[calc(100dvh-64px)] overflow-hidden bg-transparent">
            <Suspense fallback={<div className="flex h-full items-center justify-center"><p className="text-slate-500">{tCommon('loading')}</p></div>}>

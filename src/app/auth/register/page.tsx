...
function RegisterForm() {
    ...
    const [role] = useState<'CLIENT' | 'PROVIDER'>(
    ...
    const handleOAuthSignIn = (provider: 'google' | 'apple') => {
        const callbackUrl = role === 'PROVIDER' ? '/provider/onboarding' : '/dashboard';

        if (role === 'PROVIDER') {
            document.cookie = "new-user-role=PROVIDER; path=/; max-age=300";
        } else {
            document.cookie = "new-user-role=CLIENT; path=/; max-age=300";
        }
            ...
            <Card className="w-full max-w-md">
                <CardHeader>
                    <CardTitle className="text-center text-2xl">
                        {role === 'PROVIDER' ? 'Стать исполнителем' : 'Создать аккаунт'}
                    </CardTitle>
                </CardHeader>
                <CardContent>
